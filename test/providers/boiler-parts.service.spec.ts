...
describe('Auth Service', () => {
  ...
  it('should find by name', async () => {
    const part = await boilerPartsService.findOneByName('Valens tamdiu.');
  ...
  it('should find by search string', async () => {
    const parts = await boilerPartsService.searchByString('as');

    expect(parts.rows.length).toBeLessThanOrEqual(20);

    parts.rows.forEach((item) => {
      expect(item.name.toLowerCase()).toContain('as');
